...
char *parse_csv_line(const char *line) {
    ...
    for (int i = 0; line[i] != '\0'; i++) {
        if (line[i] == '"') {
            if (quote_found) { // toggle quote flag appropriately
                quote_found = false;
            } else {
                quote_found = true;
            } 
        ...
        if (line[i] == ',' && quote_found) { // don't add comma to new row if in quotes
            continue;  
        }
        else { // if not in quotes add to new row and update index
